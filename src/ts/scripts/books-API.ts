  ...
  buyNowBtn.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (btn.classList.contains("active-book_btn")) {
          ...
          btn.classList.remove("active-book_btn");
          btn.textContent = "BUY NOW";
          if(shopCounter === 0){
            shopCount.style.visibility = "hidden"
            }
      } else {
...
const fetchBooks = async (
  ...
  try {
    let response: Response = await fetch(
      `https://www.googleapis.com/books/v1/volumes?q="subject:${currentCategory}"&key= ${token} &printType=books&startIndex=${page}&maxResults=6&langRestrict=en`
    );
    ...
    if (data) {
      data.items.forEach((book: BookVolume) => {
       ...
       <div class="book-info">
           <p class="book-info_author">${authorsValidate(
           ...
           <p class="book-info_description">${descriptionValidate(
             book.volumeInfo.description
           )}</p>
           <div class="book-info_price">${priceValidate(
   ...
   `;
    ...
    buyNowBtn = document.querySelectorAll(
      ".book-info_button"
    ) as NodeListOf<HTMLElement>;
    initShopper();
  } catch (error) {
...
fetchBooks(
  currentCategory,
  "AIzaSyBbTAT12E1n5MMyhWBAQMZNl1EZFgkcKWI",
  loadMoreCount
);
